...
- **The record:** 7fr / 5fr with a 128px column gap from 900px up; the right column is sticky (by its top when it fits the screen, by its bottom when it does not, resting on the bottom edge rather than under it); stacked below 900px with 88px between columns. Rows bleed 12px into the gutter on both sides.
...
- **On sheets:** tiles are figures, not links; they lose the fixed height (and the 4:5 ratio below 900px) and pad `clamp(20px, 6%, 56px)`; a `tile--pair` variant sets a phone at 22% beside a wide screen.
...
- **Touch:** descriptions are always shown at 14px and the wash is never drawn; a keyboard on a touch device gets the standard 2px Ink ring.
...
- **Page changes:** cross-document view transitions. The old page fades out over 340ms while it steps back (`scale(.985)`, 6px blur); the new one fades in over the same 340ms on the same curve, so the two always add up to one screen of light, while it rises 12px over 520ms. The menu holds its place and the current tab's mark slides (560ms); the static cross-fades in place. If the menu is off screen when a link is followed, or will be where the next page opens (a `#section`, or Back to a page left scrolled down), it travels with the page instead. Where cross-document transitions are missing, a click fades the page out over 220ms before leaving and the next page's content rises in over 480ms; Back restores the page at full opacity. In-page anchors scroll smoothly.
