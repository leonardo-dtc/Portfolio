...
(function () {
  ...
  function frame(now) {
    ...
    if (document.hidden || !term.root.contains(document.activeElement)) { last = now; return; }
  ...
  function onKey(e) {
    if (!term || !term.root.contains(document.activeElement)) return;
    var k = norm(e);
    if (e.target && e.target.closest && e.target.closest('button') && (k === 'Enter' || k === ' ')) return;
  ...
  function open(body) {
    ...
    var root = document.createElement('div'); root.className = 'term'; root.tabIndex = 0; root.setAttribute('role','region'); root.setAttribute('aria-label','Browser games'); body.closest('.panel').classList.add('has-game');
    root.innerHTML = '<p class="term__prompt"><span class="term__p">$</span> ./games</p><pre class="term__screen" aria-live="off"></pre>' +
                     ...
                     '<div class="game-actions" aria-label="Games"><button type="button" data-program="0">Dino</button><button type="button" data-program="1">Tetris</button><button type="button" data-program="2">Snake</button><button type="button" data-game-menu>Menu</button><button type="button" data-game-close aria-label="Exit games">Exit</button></div>' +
                     '<div class="game-controls" aria-label="Game controls"><button type="button" data-key="ArrowLeft" aria-label="Left">←</button><button type="button" data-key="ArrowUp" aria-label="Up or rotate">↑</button><button type="button" data-key="ArrowDown" aria-label="Down or duck">↓</button><button type="button" data-key="ArrowRight" aria-label="Right">→</button><button type="button" data-key=" ">Space / restart</button></div>';
    body.appendChild(root);
    term = { launcher: launcher, body: body, saved: saved, root: root, screen: root.querySelector('.term__screen'), status: root.querySelector('.term__status'), hint: root.querySelector('.term__hint') };
    game = null; pick = 0; last = 0;
    addEventListener('keydown', onKey, true);
    addEventListener('keyup', onKeyUp, true);
    root.addEventListener('click', function (e) {
      var button = e.target.closest('button');
      if (!button) { onClick(e); return; }
      if (button.hasAttribute('data-program')) { run(+button.dataset.program); root.focus({preventScroll:true}); }
      else if (button.hasAttribute('data-game-menu')) { game = null; root.focus({preventScroll:true}); }
      else if (button.hasAttribute('data-game-close')) close(true);
      else if (button.hasAttribute('data-key') && e.detail === 0 && game) { game.key(button.dataset.key); if (game.keyUp) game.keyUp(button.dataset.key); }
    });
    root.addEventListener('pointerdown', function (e) {
      var button = e.target.closest('[data-key]');
      if (!button || !game) return;
      e.preventDefault(); button.setPointerCapture(e.pointerId); game.key(button.dataset.key);
    });
    function release(e) { var button = e.target.closest('[data-key]'); if (button && game && game.keyUp) game.keyUp(button.dataset.key); }
  ...
  function close(restore) {
    ...
    var panel = term.body.closest('.panel'); if (panel) panel.classList.remove('has-game');
    removeEventListener('keydown', onKey, true);
    removeEventListener('keyup', onKeyUp, true);
    if (term.root.parentNode) term.root.parentNode.removeChild(term.root);
    term.saved.forEach(function (el) { el.hidden = false; });
    if (restore && term.launcher && term.launcher.isConnected) term.launcher.focus({preventScroll:true});
    term = null; game = null;
  }
